...
LIDA 将数据摘要、分析目标探索、可视化代码生成与信息图生成组织为连续流水线，如图\ref{fig:related_lida} 所示。这类系统体现了当前自动图表生成研究的典型思路：通过模块化流程将数据理解、用户目标和图表渲染连接起来，使自然语言能够参与可视化生成过程。

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{pics/related_lida_modules.jpg}
	\caption[LIDA 自动可视化生成流程]{LIDA 自动可视化生成流程\zjutfigcaptionen{LIDA Automatic Visualization Generation Workflow~\cite{dibia_lida_2023}}}
	\label{fig:related_lida}
\end{figure}
...
混合数据集因此成为当前较具可行性的折中方案。其核心思路是将合成数据用于扩展规模和覆盖基础模式，将真实数据用于补足分布多样性和语义复杂性，从而在训练阶段形成更平衡的数据分布~\cite{liu_deplot_2023,meng_chartassistant_2024,liu_matcha_2023}。这一路径有助于降低模型对单一数据生成机制的依赖，也有助于提升跨场景迁移能力。对本研究关注的“文本到图表类型推荐”任务而言，混合数据策略同样具有现实意义，因为用户输入文本经常同时包含结构化线索与非结构化语义，需要训练数据能够覆盖两类信息的耦合关系。
	...
	图\ref{fig:related_text2chart31} 展示了 Text2Chart31 中覆盖的部分图表类型。该图说明当前数据集构建已经开始从少数基础图表扩展到更复杂的统计分布图、网格图和三维图表。对本文任务而言，图表类型覆盖越丰富，推荐系统越需要在文本意图、数据关系和图表表达方式之间建立更稳定的对应关系。

	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.72\textwidth]{pics/related_text2chart31_types.png}
		\caption[Text2Chart31 图表类型示例]{Text2Chart31 图表类型示例\zjutfigcaptionen{Example Chart Types in Text2Chart31~\cite{zadeh_text2chart31_2024}}}
		\label{fig:related_text2chart31}
	\end{figure}

	为进一步说明现有数据集与本文任务之间的差异，表\ref{tab:dataset_comparison} 对代表性图表数据集进行简要对比。可以看到，已有数据集主要服务于图表问答、说明生成和代码生成等任务，而本文构建的 ChartRWC 更强调文本描述、图表类型与候选样例之间的对应关系，能够直接支撑自然语言输入下的图表类型推荐与检索评估。

	\begin{table}[htbp]
		\centering
		\caption[代表性图表数据集对比]{代表性图表数据集对比\zjuttabcaptionen{Comparison of Representative Chart Datasets}}
		\label{tab:dataset_comparison}
		\resizebox{\textwidth}{!}{
		\begin{tabular}{llll}
			\toprule
			数据集 & 数据形态 & 主要任务 & 与本文任务的关系 \\
			\midrule
			DVQA~\cite{kafle_dvqa_2018} & 图表图像、问答对 & 图表视觉问答 & 缺少推荐语境 \\
			VisText~\cite{tang_vistext_2023} & 图表、数据表、文本说明 & 图表说明生成 & 可提供文本线索 \\
			ChartQA~\cite{masry_chartqa_2022} & 图表图像、数据表、问答对 & 图表问答推理 & 偏向问答任务 \\
			Text2Chart31~\cite{zadeh_text2chart31_2024} & 文本、代码、数据表、图表 & 文本到图表生成 & 偏向端到端生成 \\
			ChartGen~\cite{kondic_chartgen_2025} & 图表图像、代码、结构化数据 & 图表理解与生成 & 类别分布不均衡 \\
			\textbf{ChartRWC (Ours)} & \textbf{图表图像、结构化数据、元数据、文本描述} & \textbf{图表类型推荐与检索评估} & \textbf{直接面向本文任务} \\
			\bottomrule
		\end{tabular}}
	\end{table}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{pics/related_data_illustrator_interface.png}
	\caption[Data Illustrator 界面组件]{Data Illustrator 界面组件\zjutfigcaptionen{Data Illustrator Interface Components~\cite{liu_data_2018}}}
	\label{fig:related_data_illustrator}
\end{figure}
